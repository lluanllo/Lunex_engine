...
namespace Lunex {
	...
	void MaterialInstance::SetBaseAsset(Ref<MaterialAsset> asset) {
		...
		m_BaseAsset = asset;
		ResetOverrides(); // Clear overrides when changing base asset
	}
	...
	MaterialAsset::MaterialUniformData MaterialInstance::GetUniformData() const {
		...
		// Use overridden values or fall back to base asset
		...
		// Texture flags (always from base asset)
	...
	void MaterialInstance::BindTextures() const {
		// Bind all textures from base asset to their respective slots
