...
namespace Lunex {
	...
	void Renderer3D::Flush() {
		// No-op for now, immediate rendering
	}
	...
	void Renderer3D::DrawModel(const glm::mat4& transform, const Ref<Model>& model, const glm::vec4& color, int entityID) {
		...
		// Debug: Log solo primeras 5 llamadas
		static int callCount = 0;
		if (callCount < 5) {
			LNX_LOG_INFO("Renderer3D::DrawModel - EntityID: {0}, MeshCount: {1}", entityID, model->GetMeshes().size());
			callCount++;
		}
