...
namespace Lunex {
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath) {
		...
		if (in) {
			...
			// Log primeras l�neas para verificar contenido
	...
	std::string OpenGLShader::InsertDefineAfterVersion(const std::string& source, const std::string& defineLine) {
		...
		if (versionPos == std::string::npos) {
			// Si no hay #version, agregar uno est�ndar + el define
			LNX_LOG_WARN("No #version found in shader, adding default");
			return "#version 450 core\n" + defineLine + "\n" + source;
		}

		// Encontrar el final de la l�nea del #version
		size_t eolPos = source.find('\n', versionPos);

		if (eolPos == std::string::npos) {
			// Si no hay salto de l�nea, el #version est� al final
		...
		result += source.substr(eolPos + 1);                // Resto del c�digo
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources) {
		...
		for (auto&& [stage, source] : shaderSources) {
			...
			if (!cacheLoaded) {
				...
				// Log del c�digo fuente para debugging
		...
		// Verificar que tengamos datos v�lidos antes de hacer Reflect
		for (auto&& [stage, data] : shaderData) {
			if (data.empty()) {
				...
				// CR�TICO: Envolver Reflect en try-catch para capturar errores de SPIRV-Cross
				try {
					Reflect(stage, data);
				}
				catch (const std::exception& e) {
	...
	// TAMBI�N REEMPLAZA CompileOrGetOpenGLBinaries
	...
	// A�ade esto en OpenGLShader.cpp despu�s de CompileOrGetOpenGLBinaries()
