...
namespace Lunex{
	...
	void OrthographicCameraController::OnUpdate(Timestep ts) {
		if (Input::IsKeyPressed(LN_KEY_A))
			m_CameraPosition.x -= m_CameraTranslationSpeed * ts;
		else if (Input::IsKeyPressed(LN_KEY_D))
			m_CameraPosition.x += m_CameraTranslationSpeed * ts;
		
		if (Input::IsKeyPressed(LN_KEY_W))
			m_CameraPosition.y += m_CameraTranslationSpeed * ts;
		else if (Input::IsKeyPressed(LN_KEY_S))
			m_CameraPosition.y -= m_CameraTranslationSpeed * ts;
		
		if (m_Rotation) {
			if (Input::IsKeyPressed(LN_KEY_Q))
				m_CameraRotation -= m_CameraRotationSpeed * ts;
			else if (Input::IsKeyPressed(LN_KEY_E))
				m_CameraRotation += m_CameraRotationSpeed * ts;
