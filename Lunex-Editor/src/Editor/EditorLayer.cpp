...
namespace Lunex {
	...
	void EditorLayer::OnAttach() {
		...
		// Inicializar OutlineRenderer
		OutlineRenderer::Init();

		// Cargar iconos de toolbar
	...
	void EditorLayer::OnUpdate(Lunex::Timestep ts) {
		...
		// ========== OVERLAY RENDER (2D colliders, outlines 2D) ==========
		OnOverlayRender();
		...
		if (mouseX >= 0 && mouseY >= 0 && mouseX < (int)viewportSize.x && mouseY < (int)viewportSize.y) {
			...
			// Validar que el entity existe en el registry activo
			if (pixelData != -1 && !m_ActiveScene->IsValidEntity((entt::entity)pixelData))
				pixelData = -1;
	...
	// ... resto de métodos (NewScene, OpenScene, SaveScene, etc.) sin cambios
