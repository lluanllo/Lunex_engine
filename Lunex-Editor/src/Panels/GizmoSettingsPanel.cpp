...
namespace Lunex {
	void GizmoSettingsPanel::OnImGuiRender(const glm::vec2& viewportBounds,
		...
		int orientationButtons = 3;  // Global, Local

		// Calcular altura total considerando todos los botones + separador
		float totalHeight = (buttonSize * (pivotButtons + orientationButtons)) +
			(spacing * (pivotButtons + orientationButtons)) +
			12.0f +  // Espacio para separador
			(padding * 2);
		...
		// Espaciado entre secciones (sin separador visible ya que el fondo es transparente)
