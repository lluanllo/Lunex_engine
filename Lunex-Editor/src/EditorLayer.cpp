...
namespace Lunex {
	...
	void EditorLayer::OnUpdate(Lunex::Timestep ts) {
		...
		
		{
			...
			Lunex::Renderer2D::DrawRotatedQuad({ 1.0f, 0.0f }, { 0.8f, 0.8f }, -45.0f, { 0.8f, 0.2f, 0.3f, 1.0f });
			...
			Lunex::Renderer2D::DrawQuad({ 0.0f, 0.0f, -0.1f }, { 20.0f, 20.0f }, m_CheckerboardTexture, 10.0f);
			Lunex::Renderer2D::DrawRotatedQuad({ -2.0f, 0.0f, 0.0f }, { 1.0f, 1.0f }, rotation, m_CheckerboardTexture, 20.0f);
		...
		if (dockingEnabled) {
			...
			if (ImGui::BeginMenuBar()) {
				if (ImGui::BeginMenu("File")) {
					...
					if (ImGui::MenuItem("Exit")) Lunex::Application::Get().Close();
					ImGui::EndMenu();
				}
